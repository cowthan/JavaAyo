...
/* 
 *  <service android:name="com.zhongwei.environment.service.PushService" />
...
{
	/**
	 * mqttIP	112.126.68.145
	端口	1883
	主题是	IMEI+id，没分隔符，861022002223628100001，后面的100001是employeeId
	格式这样：{"checkId":65,"checkAddress":"asfadsfasf","titleName":null,"evalType":1,"checkTime":"2015-08-09"}	
	提示响铃	
	属性工作提醒界面	
	通知栏：“地址checkAddress+时间checkTime+标题titleName”
	
	08-20 16:54:15.885: I/System.out(30978): DemoPushService--subscribe topic = 865072023492054100001
米2A：860954020788508100001
	 * @param context
	 */
...
//		return Fast.service.isServiceRunning(context, PushService.class);
//	}
	
	public static String getClientId(){
		//return MQTT_CLIENT_ID + "/" + Util.getImei();
		return MQTT_CLIENT_ID + "/" + "hehe";
	}
	
	public static String getTopic(){
		//return Util.getImei() + "user-id";
		return "hehe";
	}
	
	// this is the log tag
	public static final String		TAG = "DemoPushService";
	//private static final String		DEVICE_ID = "abaass";
	// the IP address, where your MQTT broker is running.
	private static final String		MQTT_HOST = "192.168.1.108"; //"112.126.68.145"; //
	...
	public static String			MQTT_CLIENT_ID = "cleaner";
	...
	// Notification title
	public static String			NOTIF_TITLE = "Cleaner"; 	
	// Notification id
	...
	public void onCreate() {
		...
		log("DemoPushService--Creating service");
		mStartTime = System.currentTimeMillis();

		try {
			mLog = new ConnectionLog();
			Log.i(TAG, "DemoPushService--Opened log at " + mLog.getPath());
		} catch (IOException e) {
			Log.e(TAG, "DemoPushService--Failed to open log", e);
		}
	...
	private void handleCrashedService() {
		if (wasStarted() == true) {
			log("DemoPushService--Handling crashed service...");
	...
	public void onStart(Intent intent, int startId) {
		...
		log("DemoPushService--Service started with intent=" + intent);
	...
	private synchronized void start() {
		log("DemoPushService--Starting service...");
		
		// Do nothing, if the service is already running.
		if (mStarted == true) {
			Log.w(TAG, "DemoPushService--Attempt to start connection that is already active");
			return;
		}
	...
	private synchronized void stop() {
		...
		if (mStarted == false) {
			Log.w(TAG, "DemoPushService--Attempt to stop connection not active.");
			return;
		}
	...
	private synchronized void connect() {		
		log("DemoPushService--Connecting...");
		// fetch the device ID from the preferences.
		String deviceID = ""; //DEVICE_ID; //mPrefs.getString(PREF_DEVICE_ID, null);
		// Create a new connection only if the device id is not NULL
		if (deviceID == null) {
			log("DemoPushService--Device ID not found.");
		} else {
			try {
				System.out.println("DemoPushService--connet1");
				mConnection = new MQTTConnection(MQTT_HOST);//new MQTTConnection(MQTT_HOST, deviceID);
			} catch (MqttException e) {
				e.printStackTrace();
				// Schedule a reconnect, if we failed to connect
				log("DemoPushService: 出错了-" + (e.getMessage() != null ? e.getMessage() : "NULL"));
	        	if (isNetworkAvailable()) {
	...
	private synchronized void reconnectIfNecessary() {		
		if (mStarted == true && mConnection == null) {
			log("DemoPushService--Reconnecting...");
			connect();
		}
	}
	...
	private BroadcastReceiver mConnectivityChanged = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			log("DemoPushService--Connectivity changed: connected=" + hasConnectivity);
	...
	private void showNotification2(String text) {
		System.out.println("DemoPushService--" + text);
		...
		mNotifMan.notify(id, n); //(NOTIF_CONNECTED, n);
	}
	...
	private class MQTTConnection implements MqttSimpleCallback {
		...
		public MQTTConnection(String brokerHostName) throws MqttException {
	    	...
	    	System.out.println("DemoPushService--host--" + mqttConnSpec);
	        	// Create the client and connect
	        	mqttClient = MqttClient.createMqttClient(mqttConnSpec, MQTT_PERSISTENCE);
	        	String clientID = PushService.getClientId(); //MQTT_CLIENT_ID + "/" + DEVICE_ID;
	        	mqttClient.connect(clientID, MQTT_CLEAN_START, MQTT_KEEP_ALIVE);
	        	System.out.println("DemoPushService--clientID = " + clientID);
		        // register this client app has being able to receive messages
				mqttClient.registerSimpleHandler(this);
				
				// Subscribe to an initial topic, which is combination of client ID and device ID.
				String initTopic = PushService.getTopic(); //MQTT_CLIENT_ID + "/" + initTopic;
				subscribeToTopic(initTopic);
				System.out.println("DemoPushService--subscribe topic = " + initTopic);
				
				log("DemoPushService--Connection established to " + brokerHostName + " on topic " + initTopic);
		...
		private void subscribeToTopic(String topicName) throws MqttException {
			...
			if ((mqttClient == null) || (mqttClient.isConnected() == false)) {
				...
				//  a connection
				log("DemoPushService--Connection error" + "No connection");	
			} else {									
		...
		private void publishToTopic(String topicName, String message) throws MqttException {		
			if ((mqttClient == null) || (mqttClient.isConnected() == false)) {
				...
				//  a connection				
				log("DemoPushService--No connection to publish to");		
			} else {
		...
		public void connectionLost() throws Exception {
			log("DemoPushService--Loss of connection" + "connection downed");
			stopKeepAlives();
			// null itself
			mConnection = null;
		...
		public void publishArrived(String topicName, byte[] payload, int qos, boolean retained) {
			...
			showNotification(s);	
			log("DemoPushService--Got message: " + s);
		}   
		
		public void sendKeepAlive() throws MqttException {
			log("DemoPushService--Sending keep alive");
			// publish to a keep-alive topic
			publishToTopic(MQTT_CLIENT_ID + "/keepalive", Util.getImei());
		}		
	}
}