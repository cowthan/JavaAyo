 ...
 */
public class Ayo {
	public static <T> String join(List<T> list, String sep, boolean ignoreNull, String nullPlaceHolder){
		  if(list == null || list.size() == 0){
			  return "";
		  }else{
			  StringBuilder sb = new StringBuilder();
			  for(T t: list){
				  if(t == null && ignoreNull) continue;
				  
				  String s = "";
				  if(t == null){
					  s = nullPlaceHolder == null ? "null" : nullPlaceHolder;
				  }else{
					  s = t.toString();
				  }
	...
	public static <T> String join(T[] list, String sep, boolean ignoreNull, String nullPlaceHolder){
	...
	/**
	 * 拆分字符串，把字符串拆成List<String>，每个元素的最大长度是参数2指定
	 * @param s
	 * @param elementLength
	 * @return
	 */
	public static List<String> splitString(String s, int elementLength){
	...
	public static void main1(String[] args) {
		String s = "addadasdfasdfasfasfasdf";
		String s2 = "";
		List<String> list = splitString(s, 2);
		for(int i = 0; i < list.size(); i++){
			s2 += list.get(i);
			System.out.println(list.get(i));
		}
		if(s2.equals(s)){
			System.out.println("拆分成功，能还原");
		}else{
			System.out.println("拆分失败，不能还原");
		}
	}
