package com.cowthan.algrithm.std;
/*************************************************************************
...
public final class StdRandom {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < N; i++) {
            StdOut.printf("%2f "  , random());    //0到1的double
            StdOut.printf("%2d "  , uniform(100));  //0到N-1的整数
            StdOut.printf("%8.5f ", uniform(10.0, 99.0)); //  uniform(int low, int high) low到high-1之间的整数     uniform(double low, double high) low到high的实数
            StdOut.printf("%5b "  , bernoulli(.5)); //指定返回true的概率，返回true和false
            StdOut.printf("%7.5f ", gaussian()); // 正态分布，期望值0，标准差1
            StdOut.printf("%7.5f ", gaussian(9.0, .2)); //正态分布，期望值9.0，标准差0.2
            StdOut.printf("%2d "  , discrete(t)); //  对于数组a，返回i的概率为a[i]，double[] t = { .1, .3, .5, .1 };  返回0的概率是0.1，返回1的概率是0.3
            StdOut.println();
        }
    }
